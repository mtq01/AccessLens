// Builds the printable/exportable report page from saved scan data.
...
const ICONS = {
  ...
  build:
    "M13.7 7.34l1.41 1.41-9.19 9.2-1.41-1.41 9.19-9.2zm-1.83-4.76L8.34 6.1l1.41 1.41 1.06-1.06 1.41 1.41-2.12 2.12 1.41 1.41L14.93 7 11.87 2.58zM3 18.17l2.83 2.83 8.48-8.48-2.83-2.83L3 18.17z",
  check_circle:
  ...
  hearing:
    "M17 8C17 4.69 14.31 2 11 2S5 4.69 5 8c0 2.97 2.04 5.43 4.8 6.18A2.5 2.5 0 0 1 7.5 17.5h-1v2h1a4.5 4.5 0 0 0 4.5-4.5C14.32 13.77 17 11.1 17 8zm-6 4.93V6h2v2h2c0 2.21-1.79 4-4 4z",
  people:
...
const CHECKLIST_DEF = [
  {
    ...
    items: [
      {
        id: "kb-1",
        text: "Unplug your mouse. Navigate the entire page using only Tab, Shift+Tab, Enter, Space, and arrow keys.",
        wcag: "2.1.1",
      },
      {
        id: "kb-2",
        text: "Every interactive element — links, buttons, inputs, dropdowns, modals — is reachable and operable by keyboard.",
        wcag: "2.1.1",
      },
      {
        id: "kb-3",
        text: "No keyboard traps — you can always Tab away from any component.",
        wcag: "2.1.2",
      },
      {
        id: "kb-4",
        text: "Focus is never lost after closing a modal or dismissing a component.",
        wcag: "2.4.3",
      },
      {
        id: "kb-5",
        text: "Tab order follows the logical reading order of the page.",
        wcag: "2.4.3",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "sr-1",
        text: "Tested with VoiceOver (Mac: Cmd+F5) or NVDA (Windows, free download).",
        wcag: "4.1.2",
      },
      {
        id: "sr-2",
      ...
      {
        id: "sr-3",
        text: "All images have meaningful alt text that conveys their purpose.",
        wcag: "1.1.1",
      },
      {
        id: "sr-4",
        text: "Form fields announce their label, type, and any error messages when focused.",
        wcag: "1.3.1",
      },
      {
        id: "sr-5",
        text: "Dynamic content changes (loading states, errors) are announced without requiring focus.",
        wcag: "4.1.3",
      },
      {
        id: "sr-6",
        text: "Modals are announced correctly and focus is trapped inside while open.",
        wcag: "4.1.2",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "vi-1",
        text: "Content is readable and usable at 200% browser zoom with no overlap.",
        wcag: "1.4.4",
      },
      {
        id: "vi-2",
        text: "Content reflows to a single column at 400% zoom with no horizontal scrolling.",
        wcag: "1.4.10",
      },
      {
        id: "vi-3",
        text: "All error messages describe the problem in text, not just a red border or colour change.",
        wcag: "3.3.1",
      },
      {
        id: "vi-4",
        text: "Instructions do not rely on shape, colour, size, or position alone.",
        wcag: "1.3.3",
      },
      {
        id: "vi-5",
        text: "Animations can be paused or stopped. Tested with OS Reduce Motion setting.",
        wcag: "2.3.3",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "ut-1",
        text: "At least one person who uses assistive technology has tested the primary user flow end-to-end.",
        wcag: "—",
      },
      {
        id: "ut-2",
        text: "A user with low vision has tested at their preferred zoom or text size.",
        wcag: "—",
      },
      {
        id: "ut-3",
        text: "Critical flows completed successfully by a disabled user.",
        wcag: "—",
      },
    ],
  },
  {
    ...
    items: [
      {
        id: "doc-1",
        text: "An Accessibility Statement is published and linked in the footer with a contact method for users to report barriers.",
        wcag: "—",
      },
      {
        id: "doc-2",
        text: "An ACR (Accessibility Conformance Report / VPAT) has been completed for enterprise or government procurement.",
        wcag: "—",
      },
      {
        id: "doc-3",
        text: "Known issues are documented with planned fix dates, not silently ignored.",
        wcag: "—",
      },
    ],
  },
...
function getPrinciple(ruleId) {
  return RULE_TO_PRINCIPLE[ruleId] || "Robust";
}
...
function renderReport(data) {
  // Turn report data into one big HTML string and inject it into the page.
  const {
    url,
    scanDate,
    sections,
  ...
  if (violations.length === 0) {
    riskLabel = "No violations";
    gradeColor = "#16a34a";
  } else if (critical > 0) {
    riskLabel = "Fix now";
    gradeColor = "#dc2626";
  } else if (serious > 0) {
    riskLabel = "Needs work";
    gradeColor = "#d97706";
  } else if (moderate > 0) {
    riskLabel = "Minor issues";
    gradeColor = "#2563eb";
  } else {
    riskLabel = "Almost clean";
    gradeColor = "#65a30d";
  }
  ...
  if (sections.tabOrder) {
    const issues = tabOrderStops.filter(
      (s) =>
        s.hasPositiveTabindex || s.isAriaHiddenFocusable || !s.hasFocusRing,
    );
    if (issues.length > 0) {
      const TAB_FIX = {
        noFocusRing:
          "Add :focus-visible styles. Never use outline:none without a visible replacement. Use box-shadow or a styled outline with sufficient contrast.",
        positiveTabindex:
          "Remove positive tabindex values. Use tabindex='0' to include elements in natural order, tabindex='-1' for programmatic focus only.",
        ariaHiddenFocusable:
          "Add the inert attribute alongside aria-hidden, or set tabindex='-1' on all focusable children to prevent ghost focus stops.",
      };
      ...
      issues.forEach((s) => {
        let flagText = "",
          flagColor = "#d97706",
          fixKey = "noFocusRing";
        if (s.isAriaHiddenFocusable) {
          flagText = "aria-hidden but still keyboard focusable";
          flagColor = "#dc2626";
          fixKey = "ariaHiddenFocusable";
        } else if (s.hasPositiveTabindex) {
          flagText = `tabindex="${s.tabindex}" — breaks natural tab order`;
          flagColor = "#d97706";
          fixKey = "positiveTabindex";
        } else if (!s.hasFocusRing) {
          flagText = "No visible focus ring when focused";
          flagColor = "#c2410c";
          fixKey = "noFocusRing";
        }
...
window.downloadCSV = function () {
  chrome.storage.local.get("accesslens_report", (result) => {
    ...
    (data.violations || []).forEach((v) => {
      const ver = (v.tags || []).some((t) => t.includes("wcag22"))
        ? "2.2"
        : (v.tags || []).some((t) => t.includes("wcag21"))
          ? "2.1"
          : "2.0";
      const sc = (v.tags || [])
...
window.addEventListener("DOMContentLoaded", () => {
  // Retry a few times in case storage write and tab open happen close together.
  function tryLoad(attemptsLeft) {
    chrome.storage.local.get("accesslens_report", (result) => {
      ...
      try {
        ...
        document.getElementById("report-root").innerHTML =
          '<p style="color:#dc2626;padding:60px;font-size:15px;">Could not load report: ' +
          e.message +
          "</p>";
      }
    });
